...
<Tabs>
  ...
  <TabItem label="Angular">
    ...
    import { SnxProgressComponent } from '@mcp-elements/angular';
    ...
    @Component({
      ...
      template: `
        <div class="flex flex-col gap-3 w-full">
          <mcpe-progress [value]="progress()" />
          <div class="flex items-center gap-2">
            <mcpe-button size="sm" variant="outline"
              (click)="decrease()">-10</mcpe-button>
            <span class="text-sm tabular-nums">
              {{ progress() }}%
            </span>
            <mcpe-button size="sm" variant="outline"
              (click)="increase()">+10</mcpe-button>
          </div>
        </div>
      `,
    })
