...
- **Site redeploy** — repositioning + doc changes are committed-but-not-deployed; Vercel has NO git auto-deploy → run `vercel --prod --yes` from `examples/web` (see memory `vercel-deploy-manual`).
