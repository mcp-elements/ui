...
- ⏳ **RESUME HERE — redeploy site to v0.1.3** — a11y fixes are **published to npm** (css/react/vue/angular @ 0.1.3, 2026-07-01) but the **live site still serves 0.1.2** so the dialog/switch fix is NOT live. To ship: (1) bump `examples/web/package.json` deps `@mcp-elements/{react,css}` → `^0.1.3` + `pnpm install` (lockfile), (2) `pnpm --filter @mcp-elements/web build`, (3) `npx vercel@latest --prod --force --yes` from `examples/web`, (4) verify live: open the consent-dialog demo → focus enters dialog, Escape closes, focus restores. Then commit the dep bump. **Also worth**: runtime-verify the Vue/Angular dialog fix (only React was runtime-tested; Vue/Angular are build+parity only) via clean-room harnesses.
