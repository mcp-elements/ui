...
- ⏳ **RESUME HERE — two user-run commands, everything else is committed (2026-07-07)**: (1) `npm login` (old token 401s — was flagged for rotation), then from `ui/`: `pnpm -r --filter './packages/*' publish --access public --no-git-checks` → publishes all six @ **0.1.4** (metadata-only over 0.1.3); (2) from `ui/examples/web`: `npx vercel@latest --prod --force --yes` → ships mobile nav + og:image + favicon + hero fix AND the v0.1.3 a11y fixes in one deploy (deploy does NOT wait on the publish — pins resolve 0.1.3 today, 0.1.4 automatically after publish). Then verify live: hamburger opens on mobile, `/opengraph-image` 200, `/favicon.ico` 200, consent-dialog demo → focus enters, Escape closes, focus restores. **Also worth**: runtime-verify the Vue/Angular dialog fix (only React was runtime-tested; Vue/Angular are build+parity only) via clean-room harnesses.
