...
**Next concrete step** (resume here): launch posts (I7–I11) and Stage G (Vue MCP components, issue #4). Deferred config follow-ups (user said "leave for now" 2026-06-18): fix Biome `indentStyle` tab→space (lint perpetually red), registry drift-guard test, CI build of `examples/web`. (Audited: the `examples/docs` Astro site already uses the correct `npx mcp-elements add` form — the broken `@mcp-elements/cli` references were only in `examples/web`, now fixed.)
